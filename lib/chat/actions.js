...
export async function getChatSettings() {
  ...
  try {
    ...
    const ollamaBaseUrl = getConfigValue('OLLAMA_BASE_URL') || process.env.OLLAMA_BASE_URL || 'http://localhost:11434';
...
/**
 * Save Ollama base URL to config.
 * @param {string} baseUrl
 */
export async function setOllamaBaseUrl(baseUrl) {
  const user = await requireAuth();
  try {
    const { setConfigValue } = await import('../db/config.js');
    const { invalidateConfigCache } = await import('../config.js');
    setConfigValue('OLLAMA_BASE_URL', baseUrl, user.id);
    invalidateConfigCache();
    return { success: true };
  } catch (err) {
    console.error('Failed to set Ollama base URL:', err);
    return { error: 'Failed to save Ollama base URL' };
  }
}
