...
import { claudeMd, skillGuidePath, skillsDir, prototypeDir } from '../paths.js';
...
// --- Filesystem tools (sandboxed to prototype/ directory) ---

/**
 * Resolve a user-provided path safely within the prototype directory.
 * Prevents path traversal attacks (e.g., ../../etc/passwd).
 * @returns {string|null} Resolved absolute path, or null if traversal detected
 */
function resolvePrototypePath(filePath) {
  const resolved = path.resolve(prototypeDir, filePath);
  if (!resolved.startsWith(prototypeDir + path.sep) && resolved !== prototypeDir) {
    return null;
  }
  return resolved;
}

function ensurePrototypeDir() {
...
const createFileTool = tool(
  async ({ file_path, content }) => {
    ensurePrototypeDir();
    const resolved = resolvePrototypePath(file_path);
    if (!resolved) {
      return JSON.stringify({ success: false, error: 'Invalid path — must be within prototype/ directory' });
    }
    try {
      ...
      return JSON.stringify({ success: true, path: path.relative(prototypeDir, resolved) });
    } catch (err) {
  ...
  },
  {
    name: 'create_file',
    description:
      'Create or overwrite a file in the prototype/ directory. Automatically creates intermediate directories. Use this to generate code, config files, documentation, or any other files the user requests.',
    schema: z.object({
      file_path: z.string().describe('Relative path within prototype/ (e.g., "src/index.js", "README.md")'),
...
const readFileTool = tool(
  async ({ file_path }) => {
    ...
    try {
      ...
      return JSON.stringify({ success: true, path: path.relative(prototypeDir, resolved), content });
    } catch (err) {
      return JSON.stringify({ success: false, error: err.message });
    }
  },
  {
    name: 'read_file',
    description:
      'Read the contents of a file in the prototype/ directory. Returns the full file text.',
    schema: z.object({
      file_path: z.string().describe('Relative path within prototype/ (e.g., "src/index.js")'),
    }),
  }
);
...
const listFilesTool = tool(
  async ({ directory }) => {
    ensurePrototypeDir();
    const dir = directory || '.';
    const resolved = resolvePrototypePath(dir);
    if (!resolved) {
      return JSON.stringify({ success: false, error: 'Invalid path — must be within prototype/ directory' });
    }
    try {
      if (!fs.existsSync(resolved)) {
        return JSON.stringify({ success: true, path: dir, entries: [] });
      }
  ...
  },
  {
    name: 'list_files',
    description:
      'List files and directories inside the prototype/ directory. Defaults to the root of prototype/ if no subdirectory is specified.',
    schema: z.object({
      directory: z.string().optional().describe('Subdirectory to list (relative to prototype/). Omit for root.'),
    }),
  }
);
...
const deleteFileTool = tool(
  ...
  },
  {
    name: 'delete_file',
    description:
      'Delete a file or directory from the prototype/ directory. Directories are removed recursively.',
    schema: z.object({
      file_path: z.string().describe('Relative path within prototype/ to delete'),
    }),
  }
);

export { createJobTool, getJobStatusTool, getSystemTechnicalSpecsTool, getSkillBuildingGuideTool, getSkillDetailsTool, createStartCodingTool, createStartHeadlessCodingTool, createGetRepositoryDetailsTool, createFileTool, readFileTool, listFilesTool, deleteFileTool };
